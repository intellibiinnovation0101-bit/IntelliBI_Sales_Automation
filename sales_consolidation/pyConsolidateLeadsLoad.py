...
WHATSAPP_WEB_CONNECT_FIELD = "IsWhatsAppWebConnect"

MASTER_COLUMNS = (TARGET_FIELDS + TRACKING_FIELDS
                  + [VALIDATION_FIELD, RELEVANCE_FIELD, WHATSAPP_WEB_CONNECT_FIELD]
...
# Platforms that qualify a lead for the WhatsApp/Web-connect flag: it used ONLY
# Website / WhatsApp (optionally the internal IntelliBI portal) and nothing else.
# IntelliBI on its own does NOT qualify — at least one of Website / WhatsApp must
# be present.
_WA_WEB_ALLOWED_PLATFORMS = {"website", "whatsapp", "intellibi"}
_WA_WEB_CORE_PLATFORMS    = {"website", "whatsapp"}


def _is_web_whatsapp_only(platforms_used):
    """True when 'Platforms Used' contains only Website/WhatsApp (optionally plus
    IntelliBI) with at least one of Website/WhatsApp present, and NO other
    platform. Case- and whitespace-insensitive; tolerant of , ; / | separators."""
    toks = set()
    for part in re.split(r"[,;/|]", str(platforms_used or "")):
        tok = re.sub(r"\s+", "", part).lower()
        if tok:
            toks.add(tok)
    if not toks or not toks.issubset(_WA_WEB_ALLOWED_PLATFORMS):
        return False
    return bool(toks & _WA_WEB_CORE_PLATFORMS)
...
def merge_cluster(records):
    ...
    # Additional OR condition (existing terms left unchanged): a Lead Status of
    # "Irrelevant" also makes the lead Irrelevant. Compared with clean_text (the
    # standard cleanser: strip, collapse whitespace, unicode-normalise) + lower,
    ...
    # Additional Admission Status rule (an OR condition, NOT a replacement) —
    # the existing Admission Status logic above is left completely unchanged.
    # When the finalised Lead Status is "Irrelevant" (matched case-insensitively
    # and tolerant of leading/trailing/extra spaces and normal formatting via
    # clean_text — so "Irrelevant", "irrelevant", "IRRELEVANT", " Irrelevant "
    # all qualify), the Admission Status is set to "Irrelevant" too. Every other
    # Lead Status leaves the existing Admission Status value exactly as computed.
    # Deliberately placed LAST so it cannot affect any logic above that reads the
    # original Admission Status (e.g. the Lead Type "Unreachable" override or the
    # relevance flag) — nothing else in the load is changed.
    if clean_text(row.get("Lead Status", "")).lower() == "irrelevant":
        row["Admission Status"] = "Irrelevant"

    # IsWhatsAppWebConnect — flag a Website/WhatsApp lead (optionally routed via the
    # IntelliBI portal) that could not be reached. Set to "No" only when BOTH hold:
    #   1) Platforms Used is only Website/WhatsApp (optionally + IntelliBI), and
    #   2) Admission Status == "Unable to Connect" (case/space-insensitive).
    # Any other platform present, or any other Admission Status, leaves it "Yes".
    # Evaluated last so it reflects the finalised Platforms Used / Admission Status.

...
def main():
    ...
    try:
        ...
        # FALLBACK ToName (Priority 2): when the API/Address-Book sources gave no
        # attendee name, derive ToName from the To number for known internal
        # agent / escalation numbers. Runs regardless of the enrichment switches,
        # fills ONLY where ToName is still blank (never overwrites a real name),
        # and leaves unknown numbers blank (Priority 3 — never assign a wrong
        # person). ec._digits10 normalises spaces / +91 / 91 / leading 0 / int.
        if TO_NAME_FALLBACK:
            fb_applied = 0
            for r in rows:
                if not str(r.get("ToName", "")).strip():
                    _nm = TO_NAME_FALLBACK.get(ec._digits10(r.get("To", "")))
                    if _nm:
                        r["ToName"] = _nm
                        fb_applied += 1
            if fb_applied:
                log.info("Applied fallback ToName mapping to %d of %d calls.",
                         fb_applied, len(rows))

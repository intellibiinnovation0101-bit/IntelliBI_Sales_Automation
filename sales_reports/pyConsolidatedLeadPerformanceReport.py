...
def build_summary_tab(period_label, period_range, active, gen_stamp,
    ...
    if period_label == "Daily":
        ...
        # that % > 80.
        _fresh_target = fresh_nonref_target()
        ...
        _fresh_target = fresh_nonref_target()
        fresh_rgb, fresh_hex = ((TXT_GREEN, TXT_GREEN_HEX) if avg_daily_fresh >= _fresh_target
                                else (TXT_RED, TXT_RED_HEX))
        # Fresh (Non-Ref Connected) — same colour rule as Fresh (Non-Ref).
        fresh_conn_rgb, fresh_conn_hex = ((TXT_GREEN, TXT_GREEN_HEX) if avg_daily_fresh_conn >= _fresh_target
                                          else (TXT_RED, TXT_RED_HEX))
        rel_rgb, rel_hex = ((TXT_GREEN, TXT_GREEN_HEX) if rel_pct > 80
                            else (TXT_RED, TXT_RED_HEX))
        _comp_pct, _, _ = lead_completion_pct(active)
...
def _apply_metric_header_line(tab, leads):
    ...
    # Fresh — coloured against THIS counsellor's own target (lead_target_per_counsellor
    # from counsellors.json): >= target -> green, else red.
    _target = lead_target_per_counsellor()
...
def fresh_nonref_target():
    """EXPECTED Fresh (Non-Ref) leads = lead_target_per_counsellor × active
    counsellors, read fresh from config/counsellors.json on each call. 'Active'
    is matched case- and space-insensitively. Returns the legacy fallback if the
    file is missing, malformed, or produces a non-positive number."""
    try:
        with open(_COUNSELLORS_JSON, encoding="utf-8") as f:
            cfg = json.load(f)
        target = int(cfg.get("lead_target_per_counsellor", 0))
        active = sum(1 for c in cfg.get("counsellors", [])
                     if str(c.get("current_status", "")).strip().lower() == "active")
        expected = target * active
...
def build_report(period_label, period_range, df, start, end):
    ...
    for key in sorted(groups, key=lambda k: (-len(groups[k]), disp[k]))[:MAX_COUNSELLOR_TABS]:
        ...
        _apply_metric_header_line(tb, leads)       # per-counsellor metrics + Lead Completion %
...
def build_email_body(report_type, period_range, url, link_name, active, gen_stamp,
    ...
    # code all four: the two Fresh figures share the Avg-Daily-Fresh vs expected-
    # target verdict (>= expected -> green/red); the two Relevant figures share the
    # Avg-Relevant-% > 80 verdict.
    if report_type in ("Weekly", "Monthly") and start is not None and end is not None:
        ...
        fresh_hex = TXT_GREEN_HEX if avg_daily_fresh >= fresh_nonref_target() else TXT_RED_HEX
        ...
        # coded directly — Fresh (Non-Referral) green when the count >= the dynamic
        # expected target; Fresh-Relevant (Non-Referral) green when its % of that
        # count > 80.
        # Fresh-Relevant % keeps its formula: updated Fresh-Relevant over RAW Fresh (Non-Ref).
        rel_pct = (fresh_rel_nonref / fresh_nonref * 100.0) if fresh_nonref else 0.0

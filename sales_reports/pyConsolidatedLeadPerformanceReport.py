...
C_WACONNECT = "IsWhatsAppWebConnect"     # "No" = Website/WhatsApp lead that could not be connected
C_REF     = "IsReferral"
...
# ── "Connected" metrics (IsWhatsAppWebConnect) — central definitions ──────────
# A lead is "connected" unless its IsWhatsAppWebConnect is explicitly "No"
# (a Website/WhatsApp lead that could not be reached). Blank / missing / "Yes"
# all count as connected. Matching is case- and whitespace-insensitive.
def is_wa_web_connected(row):
    return s(row.get(C_WACONNECT)).strip().lower() != "no"


def count_fresh_nonref(leads):
    """Fresh (Non-Ref): fresh/new leads excluding referrals. UNCHANGED definition —
    the raw denominator used by Fresh-Relevant %."""
    return sum(1 for a in leads if a.get("_is_new") and not is_referral(a))


def count_fresh_nonref_connected(leads):
    """Fresh (Non-Ref Connected) = Fresh (Non-Ref) minus the not-connected
    (IsWhatsAppWebConnect = No) leads."""
    return sum(1 for a in leads if a.get("_is_new") and not is_referral(a)
               and is_wa_web_connected(a))


def count_fresh_rel_nonref_connected(leads):
    """Fresh-Relevant (Non-Ref), UPDATED = fresh non-ref Relevant leads minus the
    not-connected ones. This is the value now shown as 'Fresh-Relevant (Non-Ref)'
    throughout the report (header, email, counsellor tabs)."""
    return sum(1 for a in leads if a.get("_is_new") and not is_referral(a)
...
class Tab:
    def __init__(self, name):
        ...
        self.cell_fills = {}     # {(row_idx, col_idx): (rgb, hex)} per-cell overrides
...
def source_perf(active):
    ...
    for label in known + extra:
        ...
        fresh_nrc = count_fresh_nonref_connected(g)   # Fresh (Non-Ref Connected), per source
        # Order: Source | Total | Unique(Fresh) | Fresh (Non-Ref Connected) | Repeat |
        #        Interactions | Valid | Relevant | Irrelevant | Referral | GMeet |
        #        Walk-in | % Contribution
        rows.append([label, n, fresh, fresh_nrc, repeat, inter, valid, rel, irr, ref, gm, wk,
                     pct(n, source_total)])
    return rows
...
def counsellor_perf(active):
    ...
    for key in sorted(groups, key=lambda k: (-len(groups[k]), disp[k])):
        ...
        repeat = n - fresh
        fresh_nrc = count_fresh_nonref_connected(g)      # Fresh (Non-Ref Connected), per counsellor
        # Order: Counselling By | Total | Unique(Fresh) | Fresh (Non-Ref Connected) |
        #        Repeat | Interactions | Valid | Relevant | Irrelevant | Referral |
        #        GMeet | Walk-in | % Contribution
        rows.append([
...
# IntelliBI is mandatory ONLY when the lead was handled via Call / Website /
# WhatsApp. For those, IntelliBI must also be present in 'Platforms Used' for the
# lead to be Completed; otherwise Pending. Any lead that used none of those three
# platforms is Completed by default. Matching is token-based and robust to case,
# spaces, separators and ordering.
_INTELLIBI_MANDATORY_PLATFORMS = {"call", "website", "whatsapp"}


def _platform_token_set(value):
    """Normalised set of platform tokens from a 'Platforms Used' cell — lower-
    cased with all internal/edge whitespace removed, so 'Call , IntelliBI',
    'call,intellibi' and 'WhatsApp , Call' all compare equal regardless of order."""
    out = set()
    for part in re.split(r"[,;/|]", str(value or "")):
        tok = re.sub(r"\s+", "", part).lower()
        if tok:
            out.add(tok)
    return out


def lead_info_status(platforms_used):
    """'Completed' or 'Pending' for a lead's 'Platforms Used' value.
      • Contains Call / Website / WhatsApp  -> needs IntelliBI too (else Pending).
      • Contains none of those three        -> Completed (IntelliBI not required)."""
    toks = _platform_token_set(platforms_used)
    if not (toks & _INTELLIBI_MANDATORY_PLATFORMS):
        return "Completed"
    return "Completed" if "intellibi" in toks else "Pending"


def lead_completion_pct(leads):
    """(pct, completed, total) for a set of leads. Denominator = ALL leads (every
    lead is either Completed or Pending). Uses the same Platforms-Used value shown
    in the detail column (platforms_in_sequence) so the % matches the rows."""
    total = len(leads)
    if not total:
        return 0.0, 0, 0
    completed = sum(1 for a in leads
                    if lead_info_status(platforms_in_sequence(a)) == "Completed")
...
def build_summary_tab(period_label, period_range, active, gen_stamp,
    ...
    _SEP = "     |     "
    fresh_nonref = count_fresh_nonref(active)              # raw Fresh (Non-Ref), unchanged
    fresh_nonref_conn = count_fresh_nonref_connected(active)   # Fresh (Non-Ref Connected)
    # Fresh-Relevant (Non-Ref) — UPDATED to the connected value (minus not-connected).
    fresh_rel_nonref = count_fresh_rel_nonref_connected(active)
    ...
    if period_label == "Daily":
        # Daily: colour Fresh by its count vs the dynamic expected target
        # (>= expected -> green, else red); the Fresh-Relevant count and % by
        # that % > 80.
        _fresh_target = lead_target_for_period(start, end)
        fresh_rgb, fresh_hex = ((TXT_GREEN, TXT_GREEN_HEX) if fresh_nonref >= _fresh_target
                                else (TXT_RED, TXT_RED_HEX))
        # Fresh (Non-Ref Connected) — same colour rule as Fresh (Non-Ref).
        fresh_conn_rgb, fresh_conn_hex = ((TXT_GREEN, TXT_GREEN_HEX) if fresh_nonref_conn >= _fresh_target
                                          else (TXT_RED, TXT_RED_HEX))
        rel_rgb, rel_hex = ((TXT_GREEN, TXT_GREEN_HEX) if rel_pct > 80
                            else (TXT_RED, TXT_RED_HEX))
        _comp_pct, _, _ = lead_completion_pct(active)
        _comp_rgb, _comp_hex = ((TXT_GREEN, TXT_GREEN_HEX) if _comp_pct > 90
                                else (TXT_RED, TXT_RED_HEX))
        segs = [
        ...
        # share the Avg-Relevant-% > 80 verdict.
        last = min(end.date(), now_ist().date())
        ndays = max((last - start.date()).days + 1, 1)
        avg_daily_fresh = fresh_nonref / ndays
        avg_daily_fresh_conn = fresh_nonref_conn / ndays
        avg_daily_fresh_rel = fresh_rel_nonref / ndays
        _fresh_target = lead_target_for_period(start, end)
        # Colour Fresh vs the PERIOD Lead Target (period total >= period target).
        fresh_rgb, fresh_hex = ((TXT_GREEN, TXT_GREEN_HEX) if fresh_nonref >= _fresh_target
                                else (TXT_RED, TXT_RED_HEX))
        # Fresh (Non-Ref Connected) — same colour rule as Fresh (Non-Ref).
        fresh_conn_rgb, fresh_conn_hex = ((TXT_GREEN, TXT_GREEN_HEX) if fresh_nonref_conn >= _fresh_target
    ...
    if segs:
        ...
        segs = ([("Lead Target:  ", CLR_SUB_FG, HEX["SUB_FG"]),
                 (str(_fresh_target), CLR_SUB_FG, HEX["SUB_FG"]),
                 (_SEP + segs[0][0],) + tuple(segs[0][1:])]
                + segs[1:])
...
def _apply_metric_header_line(tab, leads, start=None, end=None):
    """Inject the 'Fresh | Fresh-Relevant | Fresh-Relevant % | Lead Completion %'
    metric line into the tab's (blank) second header row — same colour-run style as
    the Summary tab. Used by each individual counsellor tab so it carries its OWN
    metrics and its OWN Lead Completion %. Lead Completion % is coloured dark green
    (bold) when strictly greater than 90 %, else red (bold)."""
    _SEP = "     |     "
    fresh_nonref = count_fresh_nonref(leads)                    # raw Fresh (Non-Ref)
    fresh_nonref_conn = count_fresh_nonref_connected(leads)     # Fresh (Non-Ref Connected)
    # Fresh-Relevant (Non-Ref) — UPDATED to the connected value (minus not-connected).
    fresh_rel = count_fresh_rel_nonref_connected(leads)
    # Fresh-Relevant % keeps its formula: updated Fresh-Relevant over RAW Fresh (Non-Ref).
    rel_pct = (fresh_rel / fresh_nonref * 100.0) if fresh_nonref else 0.0
    comp_pct, _, _ = lead_completion_pct(leads)
    # Fresh — coloured against THIS counsellor's PERIOD target (per-counsellor daily
    # target × elapsed days in the report period): >= target -> green, else red.
    _target = lead_target_per_counsellor_period(start, end)
    fresh_rgb, fresh_hex = ((TXT_GREEN, TXT_GREEN_HEX) if fresh_nonref >= _target
                            else (TXT_RED, TXT_RED_HEX))
    # Fresh (Non-Ref Connected) — same colour rule as Fresh (against the counsellor target).
    fresh_conn_rgb, fresh_conn_hex = ((TXT_GREEN, TXT_GREEN_HEX) if fresh_nonref_conn >= _target
                                      else (TXT_RED, TXT_RED_HEX))
    # Fresh-Relevant count AND its % share one verdict, driven by the % (>= 80 ->
    # green, else red) — same approach as the Summary tab header.
    rel_rgb, rel_hex = ((TXT_GREEN, TXT_GREEN_HEX) if rel_pct >= 80
                        else (TXT_RED, TXT_RED_HEX))
...
# ── Fresh (Non-Ref) dynamic colour target ────────────────────────────────────
# The Fresh (Non-Ref) figure (Summary tab header + email body) is coloured
# green/red against an EXPECTED lead target read LIVE from config/counsellors.json
# — no static number:
#     expected = lead_target_per_counsellor  ×  (# counsellors whose
#                                                current_status == "Active")
# Green when the actual count (Daily) or the average-daily count (Weekly/Monthly)
# is >= expected; red when below. If the config is missing/malformed or yields a
# non-positive number, it falls back to the previous static threshold so the
# report never breaks. Only the Fresh (Non-Ref) colour uses this — every other
...
def _counsellor_employed_in_period(c, start, end):
    """True if a counsellor's employment window overlaps the report period
    [start, end]. Employment window = date_of_joining .. last_date_of_employment
    (both parsed with the existing parse_dt). A blank/unparseable
    last_date_of_employment means STILL EMPLOYED (open-ended); a blank/unparseable
    date_of_joining imposes no lower bound. Comparison is date-only, so a counsellor
    counts for a period if they were employed on any day within it."""
    doj = parse_dt(c.get("date_of_joining", ""))
    lde = parse_dt(c.get("last_date_of_employment", ""))
    if doj is not None and doj.date() > end.date():
        return False                                     # joined after the period ended
    if lde is not None and lde.date() < start.date():
        return False                                     # left before the period started
    return True


def counsellors_for_period(start=None, end=None):
    """Applicable counsellor count for the report period. WITH a period (start/end
    given), counts only the counsellors EMPLOYED during it — via date_of_joining /
    last_date_of_employment — so historical reports use the counsellors who were
    actually employed then, not today's roster. WITHOUT a period, falls back to the
    existing current_status == 'Active' count (unchanged behaviour). Reads
    config/counsellors.json live each call; 0 on any read failure."""
    try:
        with open(_COUNSELLORS_JSON, encoding="utf-8") as f:
            counsellors = (json.load(f) or {}).get("counsellors", [])
    except Exception:
        return 0
    if start is not None and end is not None:
        return sum(1 for c in counsellors if _counsellor_employed_in_period(c, start, end))
    return sum(1 for c in counsellors
               if str(c.get("current_status", "")).strip().lower() == "active")


def fresh_nonref_target(start=None, end=None):
    """LEAD TARGET = lead_target_per_counsellor × the applicable counsellor count for
    the report period. For a historical report the count is the counsellors employed
    during that period (by date_of_joining / last_date_of_employment); for the current
    period it is today's employed roster (same as the previous 'Active' count). This is
    also the value shown as the 'Lead Target' metric. Read live from
    config/counsellors.json; falls back to the legacy threshold if the file is missing,
    malformed, or yields a non-positive number."""
    try:
        with open(_COUNSELLORS_JSON, encoding="utf-8") as f:
            target = int((json.load(f) or {}).get("lead_target_per_counsellor", 0))
    except Exception:
        return _FRESH_TARGET_FALLBACK
    expected = target * counsellors_for_period(start, end)
    return expected if expected > 0 else _FRESH_TARGET_FALLBACK


def _counsellors_on_day(counsellors, d):
    """Number of counsellors employed on calendar date d (date_of_joining <= d and,
    unless last_date_of_employment is blank/open, last_date_of_employment >= d)."""
    n = 0
    for c in counsellors:
        doj = parse_dt(c.get("date_of_joining", ""))
        lde = parse_dt(c.get("last_date_of_employment", ""))
        if doj is not None and doj.date() > d:
            continue
        if lde is not None and lde.date() < d:
            continue
        n += 1
    return n


def period_days(start=None, end=None):
    """ELAPSED days in the report period: period start .. min(end, today). For a
    historical period this is the whole period; for the current (in-progress) period
    it is the days so far, so a partial week/month is compared fairly. 1 if no period."""
    if start is None or end is None:
        return 1
    last = min(end.date(), now_ist().date())
    return max((last - start.date()).days + 1, 1)


def lead_target_for_period(start=None, end=None):
    """TOTAL Lead Target for the whole report period (not a single day):
        sum over each ELAPSED day d in the period of
            lead_target_per_counsellor × (counsellors employed on day d)
    so Weekly/Monthly targets scale to the period and stay historical-aware (mid-period
    joins/leaves and the current partial period are handled day by day). Daily naturally
    yields the single-day target. Falls back to today's active roster × target when no
    period is given, and to the legacy threshold if the config is unreadable / non-positive."""
    try:
        with open(_COUNSELLORS_JSON, encoding="utf-8") as f:
            cfg = json.load(f) or {}
        per = int(cfg.get("lead_target_per_counsellor", 0))
        counsellors = cfg.get("counsellors", [])
    except Exception:
        return _FRESH_TARGET_FALLBACK
    if start is None or end is None:
        active = sum(1 for c in counsellors
                     if str(c.get("current_status", "")).strip().lower() == "active")
        return (per * active) or _FRESH_TARGET_FALLBACK
    last = min(end.date(), now_ist().date())
    d, total = start.date(), 0
    while d <= last:
        total += per * _counsellors_on_day(counsellors, d)
        d += timedelta(days=1)
    return total if total > 0 else _FRESH_TARGET_FALLBACK


def lead_target_per_counsellor():
    """The PER-counsellor Fresh target = lead_target_per_counsellor read LIVE from
    config/counsellors.json (NOT multiplied by the counsellor count). Used to
    colour the Fresh figure on each individual counsellor tab header. Falls back
    to the legacy threshold if the config is missing/malformed/non-positive."""
    try:
        with open(_COUNSELLORS_JSON, encoding="utf-8") as f:
            target = int(json.load(f).get("lead_target_per_counsellor", 0))
        return target if target > 0 else _FRESH_TARGET_FALLBACK
    except Exception:
        return _FRESH_TARGET_FALLBACK


def lead_target_per_counsellor_period(start=None, end=None):
    """PER-counsellor Lead Target for the whole report period = the per-counsellor daily
    target × elapsed days in the period. Used to colour the Fresh figure on each
    individual counsellor tab (so a weekly/monthly counsellor tab is judged against a
    weekly/monthly target, not a single day)."""
    return lead_target_per_counsellor() * period_days(start, end)
...
def write_workbook(sheets, spreadsheet_id, tabs):
    ...
    for idx, (name, tab) in enumerate(tabs.items()):
        ...
        # per-cell overrides (Lead Information Status: Completed=green / Pending=red,
        # bold) — applied after the row fill so only that one cell is recoloured.
        for (cf_ri, cf_ci), (cf_rgb, _cf_hex) in getattr(tab, "cell_fills", {}).items():
        ...
        for hi in tab.headers:
            ...
            for k, ri in enumerate(range(a, b)):
                ...
                for ci in range(1, w + 1):
                    ...
                    # per-cell override (Lead Information Status green/red, bold)
                    _cf = tab.cell_fills.get((ri, ci - 1)) if getattr(tab, "cell_fills", None) else None
                    if _cf:
                        cell.fill = fill(_cf[1])
                        cell.font = Font(bold=True)
...
def build_email_body(report_type, period_range, url, link_name, active, gen_stamp,
    ...
    # ---- Values (calculations unchanged) --------------------------------------
    # Lead Target is the PERIOD target (weekly/monthly = whole-period, historical-aware;
    # daily = single day). The Fresh figures are coloured against it.
    _lead_target = lead_target_for_period(start, end)
    fresh_nonref = count_fresh_nonref(active)                  # raw Fresh (Non-Ref)
    fresh_nonref_conn = count_fresh_nonref_connected(active)   # Fresh (Non-Ref) Connected
    ...
    if is_wm:
        ...
        avg_daily_fresh_rel = fresh_rel_nonref / nd

    # ---- Colours (Fresh vs the PERIOD Lead Target; % vs 80 / 90) ---------------
    fresh_hex      = TXT_GREEN_HEX if fresh_nonref >= _lead_target else TXT_RED_HEX
    fresh_conn_hex = TXT_GREEN_HEX if fresh_nonref_conn >= _lead_target else TXT_RED_HEX
    ...
    def _card_block(items, per_row):
        """Render items as full-width rows of at most per_row cards; the last row is
        padded with empty cells so card widths stay aligned in a grid."""
        if not items:
            return ""
        out = []
        for i in range(0, len(items), per_row):
            ...
            out.append("<table role=\"presentation\" width=\"100%\" style=\"border-collapse:"
                       "separate;margin:0 -6px 4px\"><tr>" + cells + "</tr></table>")
        return "".join(out)
    ...
    def _bar(name, value_label, pct, hexc, goal_pct, note):
    ...
    # ---- Sections (Option A) ---------------------------------------------------
    lead_volume = [("Total Leads", val("Total Leads (Fresh + Repeat)"), NAVY_HEX),
                   ("Fresh (New)", val("Fresh (New) Leads"), NAVY_HEX),
                   ("Repeat", val("Repeat Leads"), NAVY_HEX),
                   ("Referral", val("Referral Leads"), NAVY_HEX)]
    # Lead Target is the FIRST metric under Fresh Quality; "Non-Ref" lives in the
    # section heading, so the count is simply "Fresh".
  ...
  <div style="max-width:640px;margin:0 auto;background:#ffffff;border-radius:10px;
    ...
    <div style="padding:22px 28px">
      ...
      {body_sections}
      {bars}
      <p style="margin:20px 0 22px;line-height:1.5">The full report covers lead-source
